...
class AppController(QObject):
    ...
    def setActiveMode(self, mode_id: str) -> None:  # noqa: N802 - QML-facing
        ...
        """
        spec = mode_registry.find(mode_id)
        if mode_id == self._active_mode or spec is None:
            return
        if not spec.uses_player:
            # Web mode is a browser channel, not a libVLC playback mode. Stop
            # the shared engine before the WebView2 surface takes over the
            # stage, so there is never background media under the browser.
            self._engine.stop()
            self._metadata.load("")
            self._lyrics.load("")
